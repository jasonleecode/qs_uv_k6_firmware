...
///was (but wrong)
//	static const int16_t lna_short_dB[] = { (-28), (-24), (-19),    0};   //
//corrected'ish 	static const int16_t lna_dB[]       = { (-24), (-19), (-14), (
//-9), (-6), (-4), (-2), 0}; 	static const int16_t mixer_dB[]     = { ( -8), (
//-6), ( -3),    0}; 	static const int16_t pga_dB[]       = { (-33), (-27),
//(-21), (-15), (-9), (-6), (-3), 0};
    ...
    for (uint8_t lnaIdx = 0; lnaIdx < ARRAY_SIZE(lna_dB); lnaIdx++) {
      for (uint8_t mixerIdx = 0; mixerIdx < ARRAY_SIZE(mixer_dB); mixerIdx++) {
        for (uint8_t pgaIdx = 0; pgaIdx < ARRAY_SIZE(pga_dB); pgaIdx++) {
          int16_t db = lna_short_dB[lnaSIdx] + lna_dB[lnaIdx] +
                       mixer_dB[mixerIdx] + pga_dB[pgaIdx];
          GainData gainData = {{
              pgaIdx,
              mixerIdx,
              lnaIdx,
          ...
          for (i = 1; i < ARRAY_SIZE(gain_table); i++) {
            ...
            if (db == gain->gain_dB) break;
            if (db > gain->gain_dB) continue;
            if (db < gain->gain_dB) {
              if (gain->gain_dB) memmove(gain + 1, gain, 100 - i);
              gain->gain_dB = db;
...
// display update rate
static const unsigned int display_update_rate =
    250 / 10;  // max 250ms display update rate
...
void AM_fix_reset(const unsigned vfo) {  // reset the AM fixer upper
  if (vfo > 1) return;
...
void AM_fix_10ms(const unsigned vfo) {
  ...
  if (hold_counter[vfo] > 0) hold_counter[vfo]--;
  ...
  if (diff_dB > 0) {                             // decrease gain
    ...
    if (diff_dB >= 10) {  // jump immediately to a new gain setting
      ...
      while (index > 1)
        if (gain_table[--index].gain_dB <= desired_gain_dB) break;
    } else {  // incrementally reduce the gain .. taking it slow improves
              // noise/spike immunity
      if (index > 1) index--;  // slow step-by-step gain reduction
    }
...
void AM_fix_print_data(const unsigned vfo, char *s) {
  if (s != NULL && vfo < ARRAY_SIZE(gain_table_index)) {
    const unsigned int index = gain_table_index[vfo];
    sprintf(s, "%2u %4ddB %3u", index, gain_table[index].gain_dB,
            prev_rssi[vfo]);
    counter = 0;
  }
}
...
int8_t AM_fix_get_gain_diff() { return currentGainDiff; }

void AM_fix_enable(bool on) { enabled = on; }
#endif
