...
void SETTINGS_InitEEPROM(void) {
  ...
  gEeprom.BACKLIGHT_MIN =
      (Data[0] >> 4) < gEeprom.BACKLIGHT_MAX ? (Data[0] >> 4) : 0;
#ifdef ENABLE_BLMIN_TMP_OFF
  gEeprom.BACKLIGHT_MIN_STAT = BLMIN_STAT_ON;
#endif
  gEeprom.CHANNEL_DISPLAY_MODE =
      (Data[1] < 4) ? Data[1]
                    : MDF_FREQUENCY;  // 4 instead of 3 - extra display mode
  gEeprom.CROSS_BAND_RX_TX = (Data[2] < 3) ? Data[2] : CROSS_BAND_OFF;
  gEeprom.BATTERY_SAVE = (Data[3] < 5) ? Data[3] : 4;
  gEeprom.DUAL_WATCH = (Data[4] < 3) ? Data[4] : DUAL_WATCH_CHAN_A;
  gEeprom.BACKLIGHT_TIME =
      (Data[5] < ARRAY_SIZE(gSubMenu_BACKLIGHT)) ? Data[5] : 3;
  gEeprom.TAIL_TONE_ELIMINATION = (Data[6] < 2) ? Data[6] : false;
  gEeprom.VFO_OPEN = (Data[7] < 2) ? Data[7] : true;

  // 0E80..0E87
  EEPROM_ReadBuffer(0x0E80, Data, 8);
  gEeprom.ScreenChannel[0] =
      IS_VALID_CHANNEL(Data[0]) ? Data[0] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
  gEeprom.ScreenChannel[1] =
      IS_VALID_CHANNEL(Data[3]) ? Data[3] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
  ...
  gEeprom.FreqChannel[0] =
      IS_FREQ_CHANNEL(Data[2]) ? Data[2] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
  gEeprom.FreqChannel[1] =
      IS_FREQ_CHANNEL(Data[5]) ? Data[5] : (FREQ_CHANNEL_FIRST + BAND6_400MHz);
#ifdef ENABLE_NOAA
  gEeprom.NoaaChannel[0] =
      IS_NOAA_CHANNEL(Data[6]) ? Data[6] : NOAA_CHANNEL_FIRST;
  gEeprom.NoaaChannel[1] =
      IS_NOAA_CHANNEL(Data[7]) ? Data[7] : NOAA_CHANNEL_FIRST;
#endif
  ...
  {  // 0E88..0E8F
    ...
    gEeprom.FM_SelectedFrequency =
        (fmCfg.selFreq >= BK1080_GetFreqLoLimit(gEeprom.FM_Band) &&
         fmCfg.selFreq <= BK1080_GetFreqHiLimit(gEeprom.FM_Band))
            ? fmCfg.selFreq
  ...
  gEeprom.KEY_M_LONG_PRESS_ACTION =
      ((Data[0] >> 1) < ACTION_OPT_LEN) ? (Data[0] >> 1) : ACTION_OPT_NONE;
  gEeprom.KEY_1_SHORT_PRESS_ACTION =
      (Data[1] < ACTION_OPT_LEN) ? Data[1] : ACTION_OPT_MONITOR;
  gEeprom.KEY_1_LONG_PRESS_ACTION =
      (Data[2] < ACTION_OPT_LEN) ? Data[2] : ACTION_OPT_NONE;
  gEeprom.KEY_2_SHORT_PRESS_ACTION =
      (Data[3] < ACTION_OPT_LEN) ? Data[3] : ACTION_OPT_SCAN;
  gEeprom.KEY_2_LONG_PRESS_ACTION =
      (Data[4] < ACTION_OPT_LEN) ? Data[4] : ACTION_OPT_NONE;
  gEeprom.SCAN_RESUME_MODE = (Data[5] < 3) ? Data[5] : SCAN_RESUME_CO;
  gEeprom.AUTO_KEYPAD_LOCK = (Data[6] < 2) ? Data[6] : false;
  gEeprom.POWER_ON_DISPLAY_MODE =
      (Data[7] < 4) ? Data[7] : POWER_ON_DISPLAY_MODE_VOLTAGE;
  ...
  if ((Data[1] < 200 && Data[1] > 90) &&
      (Data[2] < Data[1] - 9 && Data[1] < 160 && Data[2] > 50)) {
  ...
  gEeprom.BATTERY_TYPE =
      (Data[4] < BATTERY_TYPE_UNKNOWN) ? Data[4] : BATTERY_TYPE_1600_MAH;
  ...
  gEeprom.DTMF_SEPARATE_CODE =
      DTMF_ValidateCodes((char *)(Data + 1), 1) ? Data[1] : '*';
  gEeprom.DTMF_GROUP_CALL_CODE =
      DTMF_ValidateCodes((char *)(Data + 2), 1) ? Data[2] : '#';
  ...
  gEeprom.SCAN_LIST_DEFAULT =
      (Data[0] < 3) ? Data[0] : 0;  // we now have 'all' channel scan option
  ...
  // 0D60..0E27
  EEPROM_ReadBuffer(0x0D60, gMR_ChannelAttributes,
                    sizeof(gMR_ChannelAttributes));
...
void SETTINGS_LoadCalibration(void) {
...
#ifdef ENABLE_VOX
  EEPROM_ReadBuffer(0x1F50 + (gEeprom.VOX_LEVEL * 2), &gEeprom.VOX1_THRESHOLD,
                    2);
  EEPROM_ReadBuffer(0x1F68 + (gEeprom.VOX_LEVEL * 2), &gEeprom.VOX0_THRESHOLD,
                    2);
#endif
...

  {
    ...
    gEeprom.BK4819_XTAL_FREQ_LOW =
        (Misc.BK4819_XtalFreqLow >= -1000 && Misc.BK4819_XtalFreqLow <= 1000)
            ? Misc.BK4819_XtalFreqLow
            : 0;
    ...
    //		BK4819_WriteRegister(BK4819_REG_3C,
    // gEeprom.BK4819_XTAL_FREQ_HIGH);
  }
}
...
void SETTINGS_FetchChannelName(char *s, const int channel) {
  ...
  int i;
  for (i = 0; i < 10; i++)
    if (s[i] < 32 || s[i] > 127) break;  // invalid char
...
void SETTINGS_FactoryReset(bool bIsAll) {
  ...
  for (i = 0x0C80; i < 0x1E00; i += 8) {
    if (!(i >= 0x0EE0 && i < 0x0F18) &&  // ANI ID + DTMF codes
...
#endif
  State[7] =
      (State[7] & ~(3u << 6)) | ((gSetting_backlight_on_tx_rx & 3u) << 6);
...
void SETTINGS_SaveChannel(uint8_t Channel, uint8_t VFO, const VFO_Info_t *pVFO,
  ...
  if (Mode >= 2 || IS_FREQ_CHANNEL(Channel)) {  // copy VFO to a channel
    ...
    State._8[2] =
        (pVFO->freq_config_TX.CodeType << 4) | pVFO->freq_config_RX.CodeType;
    State._8[3] = (pVFO->Modulation << 4) | pVFO->TX_OFFSET_FREQUENCY_DIRECTION;
    State._8[4] = 0 | (pVFO->BUSY_CHANNEL_LOCK << 4) |
