...
int main(void) {
  ...
  if (!gChargingWithTypeC && gBatteryDisplayLevel == 0) {
    ...
    if (gEeprom.BACKLIGHT_TIME < (ARRAY_SIZE(gSubMenu_BACKLIGHT) -
                                  1))  // backlight is not set to be always on
    ...
    if (gEeprom.POWER_ON_DISPLAY_MODE !=
        POWER_ON_DISPLAY_MODE_NONE) {  // 2.55 second boot-up screen
