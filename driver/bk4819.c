...
static const uint16_t FSK_RogerTable[7] = {0xF1A2, 0x7446, 0x61A4, 0x6544,
...
void bk4819_init(void) {
  ...
  BK4819_WriteRegister(
      BK4819_REG_19,
      0b0001000001000001);  // <15> MIC AGC  1 = disable  0 = enable
...
void BK4819_SetAGC(bool enable) {
  ...
  // 		//BK4819_WriteRegister(BK4819_REG_06, ((i << 13) | 0x2500u) +
  // 0x036u); 		BK4819_WriteRegister(BK4819_REG_06, (i & 7) << 13 | 0x4A << 7 |
  // 0x36);
  // 	}
  // }
}
...
int8_t BK4819_GetRxGain_dB(void) {
  ...
  uint8_t gainAddr =
      reg7e.gainIdx < 0 ? BK4819_REG_14 : BK4819_REG_10 + reg7e.gainIdx;
  ...
  return lnaShortTab[agcGainReg.lnaS] + lnaTab[agcGainReg.lna] +
         mixerTab[agcGainReg.mixer] + pgaTab[agcGainReg.pga];
}
...
void BK4819_SetCDCSSCodeWord(uint32_t CodeWord) {
  ...
  //                          freq(Hz) * 20.97152 for
  //                          XTAL 12.8M/19.2M/25.6M/38.4M
  //
  ...
  //
  BK4819_WriteRegister(BK4819_REG_08,
                       (0u << 15) | ((CodeWord >> 0) & 0x0FFF));  // LS 12-bits
  BK4819_WriteRegister(BK4819_REG_08,
                       (1u << 15) | ((CodeWord >> 12) & 0x0FFF));  // MS 12-bits
}

void BK4819_SetCTCSSFrequency(uint32_t FreqControlWord) {
  // REG_51 <15>  0                                 1 = Enable TxCTCSS/CDCSS 0 =
  // Disable REG_51 <14>  0                                 1 = GPIO0Input for
  // CDCSS           0 = Normal Mode.(for BK4819v3) REG_51 <13>  0 1 = Transmit
  // negative CDCSS code   0 = Transmit positive CDCSScode REG_51 <12>  0
  // CTCSS/CDCSS mode selection      1 = CTCSS                          0 =
  // CDCSS REG_51 <11>  0 CDCSS 24/23bit selection        1 = 24bit 0 = 23bit
  // REG_51 <10>  0 1050HzDetectionMode             1 = 1050/4 Detect Enable,
  // CTC1 should be set to 1050/4 Hz REG_51 <9>   0 Auto CDCSS Bw Mode 1 =
  // Disable                        0 = Enable. REG_51 <8>   0 Auto CTCSS Bw
  // Mode              0 = Enable                         1 = Disable REG_51
  // <6:0> 0 CTCSS/CDCSS Tx Gain1 Tuning     0 = min 127 = max
...
void BK4819_SetupPowerAmplifier(const uint8_t bias, const uint32_t frequency) {
  ...
  //               0 = min
  //
  //                                  280MHz       g1=1  g2=0 (-14.9dBm),  g1=4
  //                                  g2=2 (0.13dBm)
  const uint8_t gain =
      (frequency < 28000000) ? (1u << 3) | (0u << 0) : (4u << 3) | (2u << 0);
  const uint8_t enable = 1;
  BK4819_WriteRegister(BK4819_REG_36,
                       (bias << 8) | (enable << 7) | (gain << 0));
}
...
void BK4819_SetupSquelch(uint8_t SquelchOpenRSSIThresh,
  ...
  //        0 ~ 127
  //
  BK4819_WriteRegister(BK4819_REG_4F, ((uint16_t)SquelchCloseNoiseThresh << 8) |
                                          SquelchOpenNoiseThresh);
  ...
  //
  BK4819_WriteRegister(BK4819_REG_78, ((uint16_t)SquelchOpenRSSIThresh << 8) |
                                          SquelchCloseRSSIThresh);
...
void BK4819_EnableScramble(uint8_t Type) {
  ...
  BK4819_WriteRegister(BK4819_REG_71,
                       0x68DC + (Type * 1032));  // 0110 1000 1101 1100
}
...
void BK4819_SetCompander(const unsigned int mode) {
  ...
  BK4819_WriteRegister(
      BK4819_REG_29,  // (BK4819_ReadRegister(BK4819_REG_29) & ~(3u << 14)) |
                      // (compress_ratio << 14));
      (compress_ratio << 14) | (compress_0dB << 7) | (compress_noise_dB << 0));
  ...
  BK4819_WriteRegister(
      BK4819_REG_28,  // (BK4819_ReadRegister(BK4819_REG_28) & ~(3u << 14)) |
                      // (expand_ratio << 14));
      (expand_ratio << 14) | (expand_0dB << 7) | (expand_noise_dB << 0));
...
void BK4819_EnableDTMF(void) {
  ...
  //
  //	const uint16_t threshold = 24;    // default, but doesn't decode non-QS
  //radios
  const uint16_t threshold = 130;  // but 128 ~ 247 does
  BK4819_WriteRegister(
      BK4819_REG_24,  // 1 00011000 1 1 1 1110
      (1u << BK4819_REG_24_SHIFT_UNKNOWN_15) |
          (threshold << BK4819_REG_24_SHIFT_THRESHOLD) |  // 0 ~ 255
          (1u << BK4819_REG_24_SHIFT_UNKNOWN_6) | BK4819_REG_24_ENABLE |
...
// level 0 ~ 127
void BK4819_PlaySingleTone(const unsigned int tone_Hz, const unsigned int delay,
...
void BK4819_SetupAircopy(void) {
  ...
  BK4819_WriteRegister(
      BK4819_REG_5C,
      0x5665);  // Enable CRC among other things we don't know yet
  BK4819_WriteRegister(
      BK4819_REG_5D,
      0x4700);  // FSK Data Length 72 Bytes (0xabcd + 2 byte length + 64 byte
                // payload + 2 byte CRC + 0xdcba)
}
#endif
...
void BK4819_PlayDTMF(char Code) {
  ...
  const struct DTMF_TonePair tones[] = {
      ...
      {852, 1336}, {852, 1477}, {697, 1633}, {770, 1633},
  ...
  if (pSelectedTone) {
    BK4819_WriteRegister(BK4819_REG_71,
                         (((uint32_t)pSelectedTone->tone1 * 103244) + 5000) /
                             10000);  // with rounding
    BK4819_WriteRegister(BK4819_REG_72,
                         (((uint32_t)pSelectedTone->tone2 * 103244) + 5000) /
                             10000);  // with rounding
  }
}
...
void BK4819_GenTail(uint8_t Tail) {
  // REG_52
  //
  // <15>    0 Enable 120/180/240 degree shift CTCSS or 134.4Hz Tail when CDCSS
  // mode
  //         0 = Normal
  //         1 = Enable
  //
...
void BK4819_SendFSKData(uint16_t *pData) {
  ...
  for (i = 0; i < 36; i++) BK4819_WriteRegister(BK4819_REG_5F, pData[i]);
...
void BK4819_PlayRogerMDC(void) {
  ...
  for (unsigned int i = 0; i < ARRAY_SIZE(RogerMDC_Configuration); i++) {
    BK4819_WriteRegister(RogerMDC_Configuration[i].reg,
                         RogerMDC_Configuration[i].value);
  }
