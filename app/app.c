...
void (*ProcessKeysFunctions[])(KEY_Code_t Key, bool bKeyPressed,
                               bool bKeyHeld) = {
...
static void CheckForIncoming(void) {
  if (!g_SquelchLost) return;  // squelch is closed
...
static void HandleIncoming(void) {
  ...
  if (gScanStateDir == SCAN_OFF &&
      (gRxVfo->DTMF_DECODING_ENABLE || gSetting_KILLED)) {
    // DTMF DCD is enabled
    DTMF_HandleRequest();
...
#define END_OF_RX_MODE_TTE 2
  ...
  if (gScanStateDir != SCAN_OFF &&
      IS_FREQ_CHANNEL(
          gNextMrChannel)) {  // we are scanning in the frequency mode
    if (g_SquelchLost) return;
    ...
    if (!gEndOfRxDetectedMaybe
      ...
      switch (gCurrentCodeType) {
        ...
        case CODE_TYPE_REVERSE_DIGITAL:
          ...
          if (g_CxCSS_TAIL_Found) {
            if (BK4819_GetCTCType() == 1) Mode = END_OF_RX_MODE_TTE;
...
void APP_StartListening(FUNCTION_Type_t function) {
...
#ifdef ENABLE_DTMF_CALLING
  if (gSetting_KILLED) return;
#endif

#ifdef ENABLE_FMRADIO
  if (gFmRadioMode) BK1080_Init0();
#endif
  ...
  if (gScanStateDir != SCAN_OFF) CHFRSCANNER_Found();
...
#ifdef ENABLE_VOICE
  if (gVoiceWriteIndex ==
      0)  // AM/FM RX mode will be set when the voice has finished
#endif
...
uint32_t APP_SetFreqByStepAndLimits(VFO_Info_t *pInfo, int8_t direction,
  ...
  uint32_t Frequency = FREQUENCY_RoundToStep(
      pInfo->freq_config_RX.Frequency + (direction * pInfo->StepFrequency),
      pInfo->StepFrequency);

  if (Frequency >= upper)
    Frequency = lower;
  else if (Frequency < lower)
    Frequency = FREQUENCY_RoundToStep(upper - pInfo->StepFrequency,
                                      pInfo->StepFrequency);
...
uint32_t APP_SetFrequencyByStep(VFO_Info_t *pInfo, int8_t direction) {
  return APP_SetFreqByStepAndLimits(pInfo, direction,
...
static void NOAA_IncreaseChannel(void) {
  if (++gNoaaChannel > 9) gNoaaChannel = 0;
}
#endif
...
static void DualwatchAlternate(void) {
  ...
  if (gIsNoaaMode) {
    if (!IS_NOAA_CHANNEL(gEeprom.ScreenChannel[0]) ||
        !IS_NOAA_CHANNEL(gEeprom.ScreenChannel[1]))
...
#ifdef ENABLE_NOAA
  gDualWatchCountdown_10ms =
      gIsNoaaMode ? dual_watch_count_noaa_10ms : dual_watch_count_toggle_10ms;
#else
...
static void CheckRadioInterrupts(void) {
  if (SCANNER_IsScanning()) return;

  while (BK4819_ReadRegister(BK4819_REG_0C) &
         1u) {  // BK chip interrupt request
    // clear interrupts
    ...
    if (interrupts.dtmf5ToneFound) {
      const char c = DTMF_GetCharacter(
          BK4819_GetDTMF_5TONE_Code());  // save the RX'ed DTMF character
      if (c != 0xff) {
        if (gCurrentFunction != FUNCTION_TRANSMIT) {
          if (gSetting_live_DTMF_decoder) {
            size_t len = strlen(gDTMF_RX_live);
            if (len >= sizeof(gDTMF_RX_live) - 1) {  // make room
              memmove(&gDTMF_RX_live[0], &gDTMF_RX_live[1],
                      sizeof(gDTMF_RX_live) - 1);
              len--;
            }
            gDTMF_RX_live[len++] = c;
            gDTMF_RX_live[len] = 0;
            gDTMF_RX_live_timeout =
                DTMF_RX_live_timeout_500ms;  // time till we delete it
            gUpdateDisplay = true;
          }
    ...
    if (interrupts.cssTailFound) g_CxCSS_TAIL_Found = true;
    ...
    if (interrupts.cdcssFound) g_CDCSS_Lost = false;

    if (interrupts.ctcssLost) g_CTCSS_Lost = true;

    if (interrupts.ctcssFound) g_CTCSS_Lost = false;
...
#ifdef ENABLE_AIRCOPY
    if (interrupts.fskFifoAlmostFull && gScreenToDisplay == DISPLAY_AIRCOPY &&
...
void APP_Update(void) {
  ...
  if (gCurrentFunction == FUNCTION_TRANSMIT &&
      (gTxTimeoutReached ||
       SerialConfigInProgress())) {  // transmitter timed out or must de-key
  ...
  if (gCurrentFunction != FUNCTION_TRANSMIT) HandleFunction();
  ...
  if (!SCANNER_IsScanning() && gEeprom.DUAL_WATCH != DUAL_WATCH_OFF &&
      gScheduleDualWatch && gScanStateDir == SCAN_OFF && !gPttIsPressed &&
...
#ifdef ENABLE_VOX
  if (gEeprom.VOX_SWITCH) HandleVox();
#endif

  if (gSchedulePowerSave) {
    if (gPttIsPressed || gKeyBeingHeld || gEeprom.BATTERY_SAVE == 0 ||
        gScanStateDir != SCAN_OFF || gCssBackgroundScan ||
...
#endif
#ifdef ENABLE_NOAA
        || (gIsNoaaMode && (IS_NOAA_CHANNEL(gEeprom.ScreenChannel[0]) ||
                            IS_NOAA_CHANNEL(gEeprom.ScreenChannel[1])))
#endif
    ) {
  ...
  if (gPowerSaveCountdownExpired && gCurrentFunction == FUNCTION_POWER_SAVE
    ...
    if (gRxIdleMode) {
      ...
      if (gEeprom.DUAL_WATCH != DUAL_WATCH_OFF && gScanStateDir == SCAN_OFF &&
          !gCssBackgroundScan) {  // dual watch mode, toggle between the two
                                  // VFO's
        DualwatchAlternate();
        goToSleep = false;
      }
...
static void CheckKeys(void) {
  ...
  if (gPttIsPressed) {
    if (GPIO_CheckBit(&GPIOC->DATA, GPIOC_PIN_PTT) ||
        SerialConfigInProgress()) {  // PTT released or serial comms config in
                                     // progress
      if (++gPttDebounceCounter >= 3 || SerialConfigInProgress())  // 30ms
      {  // stop transmitting
        ProcessKey(KEY_PTT, false, false);
        gPttIsPressed = false;
        if (gKeyReading1 != KEY_INVALID) gPttWasReleased = true;
      }
    } else
      gPttDebounceCounter = 0;
  } else if (!GPIO_CheckBit(&GPIOC->DATA, GPIOC_PIN_PTT) &&
             !SerialConfigInProgress()) {  // PTT pressed
  ...
  if (gKeyReading0 != Key)  // new key pressed
  {
    if (gKeyReading0 != KEY_INVALID && Key != KEY_INVALID)
      ProcessKey(gKeyReading1, false,
                 gKeyBeingHeld);  // key pressed without releasing previous key
  ...
  if (gDebounceCounter == key_debounce_10ms)  // debounced new key pressed
  {
    if (Key == KEY_INVALID)  // all non PTT keys released
    {
      ...
      {
        ProcessKey(gKeyReading1, false,
                   gKeyBeingHeld);  // process last button released event
  ...
  if (gDebounceCounter < key_repeat_delay_10ms ||
      Key == KEY_INVALID)  // the button is not held long enough for repeat yet,
                           // or not really pressed
    return;

  if (gDebounceCounter ==
      key_repeat_delay_10ms)  // initial key repeat with longer delay
  {
    if (Key != KEY_PTT) {
      gKeyBeingHeld = true;
  ...
  {
    if (Key == KEY_UP ||
        Key == KEY_DOWN)  // fast key repeats for up/down buttons
    {
      gKeyBeingHeld = true;
    ...
    if (gDebounceCounter < 0xFFFF) return;
...
void APP_TimeSlice10ms(void) {
  ...
  if (gVoxPauseCountdown > 0) gVoxPauseCountdown--;
#endif
    ...
    if (gAlarmState == ALARM_STATE_TXALARM ||
      ...
      if (gEeprom.ALARM_MODE == ALARM_MODE_TONE &&
        ...
        if (gAlarmState == ALARM_STATE_TXALARM) {
          ...
          if (gEeprom.TAIL_TONE_ELIMINATION) RADIO_SendCssTail();
...
void APP_TimeSlice500ms(void) {
  ...
  if (gKeypadLocked > 0)
    if (--gKeypadLocked == 0) gUpdateDisplay = true;
  ...
  if (gDTMF_RX_live_timeout > 0) {
    ...
    if (center_line == CENTER_LINE_DTMF_DEC ||
        center_line == CENTER_LINE_NONE)  // wait till the center line is free
                                          // for us to use before timing out
#endif
    {
      if (--gDTMF_RX_live_timeout == 0) {
  ...
  if (gDTMF_RX_timeout > 0)
    if (--gDTMF_RX_timeout == 0) DTMF_clear_RX();
#endif
  ...
  if (gBacklightCountdown_500ms > 0 && !gAskToSave &&
      !gCssBackgroundScan
      // don't turn off backlight if user is in backlight menu option
      && !(gScreenToDisplay == DISPLAY_MENU &&
           (UI_MENU_GetCurrentMenuId() == MENU_ABR ||
  ...
  if (gCurrentFunction != FUNCTION_TRANSMIT) {
    if ((gBatteryCheckCounter & 1) == 0) {
      BOARD_ADC_GetBatteryInfo(&gBatteryVoltages[gBatteryVoltageIndex++],
                               &gBatteryCurrent);
      if (gBatteryVoltageIndex > 3) gBatteryVoltageIndex = 0;
  ...
  if ((gBatteryCheckCounter & 3) == 0) {
    if (gChargingWithTypeC || gSetting_battery_text > 0) gUpdateStatus = true;
#ifdef ENABLE_SHOW_CHARGE_LEVEL
    if (gChargingWithTypeC) gUpdateDisplay = true;
#endif
  }
  ...
  if (!gCssBackgroundScan && gScanStateDir == SCAN_OFF && !SCANNER_IsScanning()
    ...
    if (exit_menu) {
      ...
      /*
         ...
         VFO_CONFIGURE_RELOAD); RADIO_ConfigureChannel(1, VFO_CONFIGURE_RELOAD);
  ...
  if (gDTMF_CallState != DTMF_CALL_STATE_NONE &&
    ...
    if (gDTMF_CallState == DTMF_CALL_STATE_RECEIVED &&
        gEeprom.DTMF_auto_reset_time >= DTMF_HOLD_MAX) {
      gDTMF_CallState =
          DTMF_CALL_STATE_RECEIVED_STAY;  // keep message on-screen till a key
                                          // is pressed
    } else {
      gDTMF_CallState = DTMF_CALL_STATE_NONE;
    }
...
static void ProcessKey(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld) {
  if (Key == KEY_EXIT && !BACKLIGHT_IsOn() &&
      gEeprom.BACKLIGHT_TIME > 0) {  // just turn the light on for now so the
                                     // user can see what's what
    BACKLIGHT_TurnOn();
    gBeepToPlay = BEEP_NONE;
    return;
  }
  ...
  if (gEeprom.AUTO_KEYPAD_LOCK) gKeyLockCountdown = 30;  // 15 seconds
  ...
  if (!bKeyPressed) {  // key released
    ...
    if (flagSaveChannel) {
      ...
      if (!SCANNER_IsScanning() && gVfoConfigureMode == VFO_CONFIGURE_NONE)
        // gVfoConfigureMode is so as we don't wipe out previously setting this
        // variable elsewhere
    ...
    if (  // not when PTT and the backlight shouldn't turn on on TX
        ...
        && !(gScreenToDisplay == DISPLAY_MENU &&
             (m == MENU_ABR || m == MENU_ABR_MAX || m == MENU_ABR_MIN))) {
      BACKLIGHT_TurnOn();
    }
    ...
    if (Key == KEY_EXIT && bKeyHeld) {  // exit key held pressed
      ...
      if (gMonitor) ACTION_Monitor();  // turn off the monitor
  ...
  bool lowBatPopup =
      gLowBattery && !gLowBatteryConfirmed && gScreenToDisplay == DISPLAY_MAIN;
  ...
  if ((gEeprom.KEY_LOCK || lowBatPopup) &&
    ...
    // KEY_MENU has a special treatment here, because we want to pass hold event
    // to ACTION_Handle but we don't want it to complain when initial press
    // happens we want to react on realese instead
    else if (Key != KEY_SIDE1 && Key != KEY_SIDE2 &&  // pass side buttons
             !(Key == KEY_MENU && bKeyHeld))          // pass KEY_MENU held
    {
      if ((!bKeyPressed || bKeyHeld ||
           (Key == KEY_MENU && bKeyPressed)) &&  // prevent released or held,
                                                 // prevent KEY_MENU pressed
  ...
  if (gCurrentFunction == FUNCTION_TRANSMIT) {
...
#endif
    {
      ...
      if (!bKeyPressed || bKeyHeld) {
        ...
        if (gEeprom
                .DTMF_SIDE_TONE) {  // user will here the DTMF tones in speaker
          AUDIO_AudioPathOn();
          gEnableSpeaker = true;
        }
...
Skip:
  ...
  if (gRequestSaveChannel > 0) {  // TODO: remove the gRequestSaveChannel, why
    ...
    if (!bKeyHeld) {
      ...
      if (!SCANNER_IsScanning() && gVfoConfigureMode == VFO_CONFIGURE_NONE)
        ...
        // variable elsewhere
        gVfoConfigureMode = VFO_CONFIGURE;
    } else {  // this is probably so settings are not saved when up/down button
              // is held and save is postponed to btn release
  ...
  if (gFlagReconfigureVfos) {
    ...
    if (gMonitor) ACTION_Monitor();  // 1of11
  }
