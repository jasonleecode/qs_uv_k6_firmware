...
void toggle_chan_scanlist(
    void) {  // toggle the selected channels scanlist setting
  ...
  if (!IS_MR_CHANNEL(gTxVfo->CHANNEL_SAVE)) {
    ...
    if (gScanRangeStart > gScanRangeStop) SWAP(gScanRangeStart, gScanRangeStop);
#endif
    return;
  }
  ...
  switch (Key) {
    ...
    case KEY_1:
      if (!IS_FREQ_CHANNEL(gTxVfo->CHANNEL_SAVE)) {
        ...
        if (IS_MR_CHANNEL(
                gEeprom.ScreenChannel[vfo])) {  // copy channel to VFO,
                                                // then swap to the VFO

          gEeprom.ScreenChannel[vfo] =
              FREQ_CHANNEL_FIRST + gEeprom.VfoInfo[vfo].Band;
    ...
    case KEY_3:
      ...
      if (beep) gBeepToPlay = BEEP_500HZ_60MS_DOUBLE_BEEP_OPTIONAL;
...
static void MAIN_Key_DIGITS(KEY_Code_t Key, bool bKeyPressed, bool bKeyHeld) {
  ...
  if (!gWasFKeyPressed) {  // F-key wasn't pressed
    ...
    if (IS_MR_CHANNEL(
            gTxVfo->CHANNEL_SAVE)) {  // user is entering channel number
    ...
    //		#endif
    if (IS_FREQ_CHANNEL(
            gTxVfo->CHANNEL_SAVE)) {  // user is entering a frequency
      ...
      if (Frequency < frequencyBandTable[0].lower) {
        ...
        Frequency =
            (Frequency < center) ? BX4819_band1.upper : BX4819_band2.lower;
      ...
      return;
                 ...
                 gTxVfo->CHANNEL_SAVE)) {  // user is entering NOAA channel
      if (gInputBoxIndex != 2) {
#ifdef ENABLE_VOICE
        gAnotherVoiceID = (VOICE_ID_t)Key;
#endif
...
static void MAIN_Key_EXIT(bool bKeyPressed, bool bKeyHeld) {
  if (!bKeyHeld && bKeyPressed) {  // exit key pressed
    ...
    if (gDTMF_CallState != DTMF_CALL_STATE_NONE &&
        gCurrentFunction !=
            FUNCTION_TRANSMIT) {  // clear CALL mode being displayed
...
#ifdef ENABLE_VOICE
        if (gInputBoxIndex == 0) gAnotherVoiceID = VOICE_ID_CANCEL;
#endif
      } else {
...
static void MAIN_Key_STAR(bool bKeyPressed, bool bKeyHeld) {
  ...
  if (!gWasFKeyPressed)  // pressed without the F-key
  {
    if (gScanStateDir == SCAN_OFF
      ...
      gBeepToPlay = BEEP_1KHZ_60MS_OPTIONAL;
      memcpy(gDTMF_InputBox, gDTMF_String,
             MIN(sizeof(gDTMF_InputBox), sizeof(gDTMF_String) - 1));
      gDTMF_InputBox_Index = 0;
...
static void MAIN_Key_UP_DOWN(bool bKeyPressed, bool bKeyHeld,
  ...
  if (bKeyHeld || !bKeyPressed) {    // key held or released
    if (gInputBoxIndex > 0) return;  // leave if input box active

    if (!bKeyPressed) {
      if (!bKeyHeld || IS_FREQ_CHANNEL(Channel)) return;
...
#endif
    {
      ...
      Channel = NOAA_CHANNEL_FIRST +
                NUMBER_AddWithWraparound(
                    gEeprom.ScreenChannel[gEeprom.TX_VFO] - NOAA_CHANNEL_FIRST,
                    Direction, 0, 9);
