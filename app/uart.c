...
static void SendReply(void *pReply, uint16_t Size) {
  ...
  if (bIsEncrypted) {
    ...
    unsigned int i;
    for (i = 0; i < Size; i++) pBytes[i] ^= Obfuscation[i % 16];
  }
...
static bool IsBadChallenge(const uint32_t *pKey, const uint32_t *pIn,
  ...
  for (i = 0; i < 4; i++)
    if (IV[i] != pResponse[i]) return true;
...
static void CMD_051B(const uint8_t *pBuffer) {
  ...
  if (pCmd->Timestamp != Timestamp) return;
  ...
  if (bHasCustomAesKey) bLocked = gIsLocked;

  if (!bLocked) EEPROM_ReadBuffer(pCmd->Offset, Reply.Data.Data, pCmd->Size);
  ...
  if (!bIsLocked) {
    ...
    for (i = 0; i < (pCmd->Size / 8); i++) {
      ...
      if (Offset >= 0x0F30 && Offset < 0x0F40)
        if (!gIsLocked) bReloadEeprom = true;
    ...
    if (bReloadEeprom) SETTINGS_InitEEPROM();
  }
...
static void CMD_052D(const uint8_t *pBuffer) {
  ...
  if (!bIsLocked) {
    ...
    if (bIsLocked) gTryCount++;
  }

  if (gTryCount < 3) {
    if (!bIsLocked) gTryCount = 0;
  } else {
    gTryCount = 3;
    bIsLocked = true;
  }
...
static void CMD_052F(const uint8_t *pBuffer) {
  ...
  gEeprom.VfoInfo[0].TX_OFFSET_FREQUENCY_DIRECTION =
...
bool UART_IsCommandAvailable(void) {
  ...
  while (1) {
    ...
    if (UART_DMA_Buffer[DMA_INDEX(gUART_WriteIndex, 1)] == 0xCD) break;
  ...
  if (CommandLength < (Size + 8)) return false;
  ...
  if (UART_Command.Header.ID == 0x6902) bIsEncrypted = true;
